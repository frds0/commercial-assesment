...
class M_Kasie extends CI_Model
{
    ...
    function JumlahDiproses()
    {
        $query = $this->db->query('SELECT * FROM tbl_permintaan WHERE status= "On Progress" or status="Pending"');
        $diproses = $query->num_rows();
        return $diproses;
    }

    function JumlahApprove()
    {
        $query = $this->db->query('SELECT * FROM tbl_permintaan WHERE status= "Approved"');
        $approve = $query->num_rows();
        return $approve;
    }
    ...
    function JumlahDeclined()
    {
        $query = $this->db->query('SELECT * FROM tbl_permintaan WHERE status= "Declined"');
        $revisi = $query->num_rows();
        return $revisi;
    }
    ...
    function GetData($id)
    {
        $query = $this->db->query("SELECT a.id_permintaan,a.lokasi, a.npp, a.nps, a.status, a.form_revisi,a.pemilihan_user,a.nama_user,a.sub_lokasi,a.luas_wilayah,a.kegiatan_akan, a.deskripsi,a.file_ppt,a.created_date, a.created_by, a.modified_by, a.modified_date,b.nama as nama_approval FROM tbl_permintaan a LEFT JOIN tbl_user b ON a.approval_by = b.npk WHERE id_permintaan = '$id'");
        return $query->result();
    }
    ...
    // function GetPertanyaanJoinWhere($id_kategori)
    // {
    //     $this->db->select('*')->from('ms_pertanyaan_ceklis a')->join('ms_kategori_ceklis b', 'a.id_kategori = b.id_kategori', 'left')->where('b.id_kategori', $id_kategori)->order_by('kategori_ceklis', 'asc');
    //     $rs = $this->db->get();
    //     $jml = $rs->num_rows();
    //     return $jml;
    // }
    ...
    // function JoinPertanyaanCeklis($id)
    // {
    //     $this->db->select('ms_pertanyaan_ceklis.id_kategori, ms_kategori_ceklis.kategori_ceklis')->from('ms_pertanyaan_ceklis')->join('ms_kategori_ceklis', 'ms_pertanyaan_ceklis.id_kategori = ms_kategori_ceklis.id_kategori')->where('id_kategori', $id);
    //     $rs = $this->db->get();
    //     return $rs->result();
    // }
    // Wawancara
    ...
    {
        $this->db->select('*')->from('tbl_cctv')->where('id_permintaan', $id);
        $rs = $this->db->get();
        return $rs->result();
    }
